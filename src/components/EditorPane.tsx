    ...
    highlightRanges && highlightRanges.length > 0
      ? (() => {
          ...
          highlightRanges.forEach((range) => {
            if (
              range.line < minLine ||
              (range.line === minLine && range.column < minColumn)
            ) {
              minLine = range.line;
            ...
            if (
              range.line > maxLine ||
              (range.line === maxLine && range.endColumn > maxColumn)
            ) {
              maxLine = range.line;
  ...
  useEffect(() => {
    ...
    const update = async () => {
      try {
        ...
        const h = await shiki.codeToHtml(value || '', {
          lang: language,
          theme: 'github-light',
        });
    ...
    <div className="relative w-full h-full">
      ...
      >
        {Array.from({ length: lines }).map((_, i) => {
          ...
          const isHighlighted =
            mergedRange &&
            lineNum >= mergedRange.startLine &&
            lineNum <= mergedRange.endLine;
          return (
            <div
              key={i}
              style={{ color: isHighlighted ? '#d97706' : undefined }}
            >
              {lineNum}
            </div>
          );
